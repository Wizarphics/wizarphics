...
use wizarphics\wizarframework\interfaces\CookieInterface;
 ...
 * ```
 */
class Cookie implements ArrayAccess, CookieInterface
{
    /**
     * @var string
     */
    ...
    public static function setDefaults($config = [])
    {
        ...
        // This array union ensures that no empty defaults will occur.
        self::$defaults = $config + $oldDefaults;
    ...
    final public function __construct(string $name, string $value = '', array $options = [])
    {
        ...
        $options['expires'] = static::createValidExpiresTimestamp($options['expires']);
        ...
        $this->name     = $name;
        $this->value    = $value;
        $this->expires  = static::createValidExpiresTimestamp($options['expires']);
        $this->path     = $path;
        $this->domain   = $domain;
        $this->secure   = $secure;
    ...
    public function withExpires($expires)
    {
        ...
        $cookie->expires = static::createValidExpiresTimestamp($expires);
     ...
     */
    protected static function createValidExpiresTimestamp($expires = 0): int
    {
        ...
        if (!is_string($expires) && !is_int($expires)) {
            throw new RuntimeException(__('Cookie.invalidExpiresTime', [gettype($expires)]));
        }
        ...
        if (!is_numeric($expires)) {
            ...
            if ($expires === false) {
                throw new RuntimeException(__('Cookie.invalidExpiresValue'));
            }
        }
    ...
    protected function validateName(string $name, bool $raw): void
    {
        if ($raw && strpbrk($name, self::$reservedCharsList) !== false) {
            throw new RuntimeException(__('Cookie.invalidCookieName', [$name]));
        }

        if ($name === '') {
            throw new RuntimeException(__('Cookie.emptyCookieName'));
        }
    }
    ...
    protected function validatePrefix(string $prefix, bool $secure, string $path, string $domain): void
    {
        if (strpos($prefix, '__Secure-') === 0 && !$secure) {
            throw new RuntimeException(__('Cookie.invalidSecurePrefix'));
        }

        if (strpos($prefix, '__Host-') === 0 && (!$secure || $domain !== '' || $path !== '/')) {
            throw  new RuntimeException(__('Cookie.invalidHostPrefix'));
        }
    }
    ...
    protected function validateSameSite(string $samesite, bool $secure): void
    {
        ...
        if (!in_array(strtolower($samesite), self::ALLOWED_SAMESITE_VALUES, true)) {
            throw new RuntimeException(__('Cookie.invalidSameSite', [$samesite]));
        }

        if (strtolower($samesite) === self::SAMESITE_NONE && !$secure) {
            throw new RuntimeException(__('Cookie.invalidSameSiteNone'));
        }
    }
}
